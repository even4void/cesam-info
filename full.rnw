...
d'intérêt est de savoir si la survie entre les deux groupes de patients, et
on négligera les autres variables présentes dans le fichier de données
\texttt{prostate.dat}. 
\begin{description}
...
variable \texttt{qtt}, d'où l'option \texttt{range(2, 5)}.
\begin{verbatim}
...
seules observations du groupe fumeur (\texttt{tab == 1} ou \texttt{tab ==
  "F"}) ne pose pas de problème car on vient de le voir dans l'application
