...
variable • inspecter la distribution d'une variable
...
\chapter{Statistiques descriptives et estimation}
...
\paragraph{Références : } \noindent
METHO, ED 2 et 3\\ RSTA, Planche 2
\end{minipage}} 
\end{center}
...
%--------------------------------------------------------------- Chapter 03 --
\chapter{Comparaisons de deux variables}
...
\paragraph{Références : } \noindent
METHO, ED 2 et 3\\ RSTA, Planche 3
...
effet soporifique. Vérifier l'exactitude de ces conclusions à l'aide d'un
test de Student.
\begin{sol}
...
hydrobromide et D. hyoscyamine hydrobromide.

Le résultat du test $t$ est reporté ci-dessous :
<<3-2b>>=
...
observée (au niveau du gain horaire de sommeil) est en accord avec les
...
%--------------------------------------------------------------- Chapter 04 --
\chapter{Analyse de variance et plans d'expérience}
...
\item[(a)] Tester l'hypothèse nulle selon laquelle l'âge de diagnostic ne varie
  pas selon le génotype à l'aide d'une ANOVA. 
...
\item[(c)] Estimer les paramètres de la régression linéaire où cette fois la
  ...
  restant le poids des bébés. Comparer avec les résultats obtenus à partir
  d'une ANOVA à un facteur (ethnicité).
...
@
\cmd{lm}\cmd{summary.lm}
Toutefois, il convient de rappeler comment interpréter les résultats d'un
tel modèle de régression linéaire avec prédicteur catégoriel.
...
sont appelés contrastes de traitement. 
<<ex5-4h>>=
...
\chapter{Mesures d'association en épidémiologie et régression logistique}
% FIXME:
...
\item[(b)] Indiquer les valeurs des odds pré et post-test.
\item[(c)] Quelle est la valeur de l'aire sous la courbe pour les données
  reportées ?
\end{description}
\begin{sol}
...
<<ex6-2a>>=
tab <- matrix(c(327,115,208,815), nr=2, dimnames=list(EST=c("-","+"), CAD=c("-","+")))
tab
@   
\cmd{matrix}

Il serait tout à fait possible de calculer en très peu d'opérations
arithmétiques avec \R toutes les quantités demandées. Cependant, il existe
des fonctionnalités 
...
\chapter{Élements du langage, statistiques descriptives, estimation}

Seuls les corrections pour les exercices~\ref{exo:2.3}, xx, xx et xx sont
proposées. 
...
fournit pas la médiane. Pour obtenir l'intervalle inter-quartile, on peut le
calculer ainsi :
\begin{verbatim}
. egen lwtiqr = iqr(lwt)
. di lwtiqr
...
\begin{verbatim}
. egen agedec = cut(age), group(3) label
\end{verbatim}
...
Seuls les corrections pour les exercices~\ref{exo:3.1}, xx, xx et xx sont
proposées. 
...
les saisir manuellement à l'aide de la commande \verb|- input -|. Dans le
cas présent, on va donc créer deux variables, \texttt{DHH} et \texttt{LHH},
correspondant aux temps de sommeil enregistrées pour les molécules
D. hyoscyamine hydrobromide et L. hyoscyamine hydrobromide, respectivement.
\begin{verbatim}
. clear all
. input DHH LHH
...
indiqué ci-après.
% FIXME:
% Problème avec l'odds-ratio par rapport à Agresti
\begin{verbatim}
. tabodds infarctus traitement [fweight=N], or

---------------------------------------------------------------------------
  traitement |  Odds Ratio       chi2       P>chi2     [95% Conf. Interval]
-------------+-------------------------------------------------------------
     Placebo |    1.000000          .           .              .          .
    Aspirine |    0.640903       2.13       0.1446      0.350818   1.170853
---------------------------------------------------------------------------
...
. list in 1/5
  ...
  5. |  A   29.6 |
     +-----------+
\end{verbatim}
%% Pour cela il est plus commode de changer le nom des trois variables de
%% façon à indiquer le \no\ de classe. Par exemple, on peut renommer les trois
%% variables ainsi : \texttt{G1} (\texttt{A}), \texttt{G2} (\texttt{B}) et
%% \texttt{G3} (\texttt{C}). Il est également nécessaire de générer un
%% identifiant unique, disons \texttt{ID}, pour chaque ligne (en l'occurence,
%% ceux-ci ne correspondent pas aux unités statistiques). Il est alors possible
%% de transformer le tableau de données au format désiré à l'aide de la
%% commande \texttt{reshape} :
%% \begin{verbatim}
%% . reshape long G, i(ID)
%% (note: j = 1 2 3)
...
pas compatible avec ce format, il est néecssaire de recoder les données
comme dans l'exercie~\ref{exo:3.5stata}. Par exemple,
\begin{verbatim}
...
% Colophon
%\vfill
{\rm\textcolor{Scode}{\scriptsize Last updated: \today. \LaTeX\ +
    pdf\TeX\ and R~\Sexpr{paste(R.Version()$major,R.Version()$minor,sep=".")}}.}
