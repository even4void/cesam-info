% Time-stamp: <2012-12-26 20:21:06 chl>
...
\section*{Objectifs}
L'objectif du cours \textbf{Méthodologie Statistique avec R et Stata} est de
fournir les bases de l'analyse statistique des données biomédicales à partir
d'un logiciel statistique.

\section*{Déroulement des séances}
Chaque séance sera composée d'une brève présentation des commandes R ou
Stata utiles pour le thème abordé, suivi d'une présentation interactive des
solutions aux exercices d'entraînement proposés. Celles-ci ne se veulent pas
exhaustives mais visent à orienter le practicien vers les idées maitresses
de l'analyse statistique de données à l'aide d'un logiciel statistique. Il
existe toujours plusieurs manières de résoudre un problème, et d'autres
solutions sont possibles pour aborder les problèmes. Dans un souci de
clarté, des solutions simples sont proposées. Le lecteur est invité à en
chercher d'autres, plus économiques ou plus pratiques.

Les premières séances sont généralement consacrées à la manipulation des
données et aux statistiques descriptives. Ces dernières sont rarement
présentées dans les séances suivantes (ANOVA, régression linéaire,
régression logistique, analyse des données de survie) mais le lecteur est
invité à appliquer les méthodes de description des données numériques et
catégorielles vues dans les premières séances pour chaque nouveau
problème. En effet, même pour les statistiques inférentielle une bonne
analyse descriptive univariée reste indispensable !
...
<<ex1-6a>>=
biolum <- read.table("bioluminescence.dat", header=FALSE)
biolum
@   
...
<<ex1-6d>>=
save(biolum, file="bioluminescence.RData")
@ 
\end{sol}
\end{exo}
...
\begin{description}
\item[\bf 1.6]
Soit l'histogramme de fréquences suivant :\\
...
\item[\bf 1.1] On souhaite importer les données sous R en utilisant la
  commande \texttt{read.table}. Quelles options faut-il renseigner ? \\[1em]
  \begin{minipage}{.6\linewidth}
    \begin{description}
...
rapport largeur/hauteur et de la couleur) ?
\begin{center}
...
bébés des deux sexes à la naissance.
\begin{verbatim}
...
\item[\bf 1.8] 
Le statisticien décide de réaliser un test non-paramétrique à l'aide de la commande
\begin{verbatim}
...
\item[\bf 2.2] Quelle commande permet d'obtenir la distribution marginale
  chez les femmes (totaux lignes) ? \\[1em]
  \begin{minipage}{.6\linewidth}
    \begin{description}
...
\end{description}  
\noindent Vous répondez :\\ \Square{A}\quad \Square{B}\quad \Square{C}\quad \Square{NSP}
\end{description}

\begin{description}
\item[\bf 2.5]
Voici le résultat d'un test statistique :
\begin{verbatim}
data:  smoke 
p-value = 1.669e-06
...
\begin{description}
\item[\bf 1.1] Quelle commande a permis de produire le résultat présenté ci-dessus ? \\[1em]
  \begin{minipage}{.6\linewidth}
    \begin{description}
  ...
  \begin{minipage}{.35\linewidth}
    ...
    \Square{NSP}
  \end{minipage}
\item[\bf 1.2] Les données comportent-elles des valeurs manquantes ? \\[1em] 
  \begin{minipage}{.6\linewidth}
    \begin{description}
    \item[A.] Oui.
    \item[B.] Non.
...
\item[\bf 1.3] On désire recoder la variable \texttt{gain} en variable
  ...
  commande est la plus appropriée ? \\[1em]
    \begin{description}
    ...
    Vous répondez :\\ \Square{A}\quad \Square{B}\quad \Square{C}\quad
    \Square{D}\quad \Square{NSP}
  \item[\bf 1.4] Quelle commande a permis de fournir les résultats suivants :
\begin{verbatim}
...
Quelle est la part de variance expliquée par ce modèle ?    
    ...
    \Square{NSP}
  \item[\bf 1.6]
Les données individuelles sont représentées dans le graphique suivant.
\begin{center}
...
\begin{description}
  \item[\bf 2.1] Pour calculer le rapport entre la variance du groupe 3 et
    celle du groupe 1, on peut utiliser la commande
    \begin{description}
  ...
  \item[\bf 2.2] On souhaite représenter la distribution de la pression
    ...
    groupes. Quelle commande est la plus appropriée ?
    \begin{description}
...
\item[\bf 1.1] On souhaite afficher un diagramme de dispersion où l'âge
  ...
  l'axe vertical (ordonnées). Quelle commande peut-on utiliser ? \\[1em]
  \begin{minipage}{.6\linewidth}
    \begin{description}
...
\item[\bf 1.2] On souhaite vérifier l'étendue des âges (minimum et maximum)
  ...
  de répondre à cette question ? \\[1em] 
  \begin{minipage}{.6\linewidth}
    \begin{description}
    \item[A.] \verb|with(d, tapply(age, raceth, c(mean, sd)))|
    \item[B.] \verb|with(d, tapply(age, raceth, function(x) min(x), max(x)))|
    \item[C.] \verb|with(d, tapply(age, raceth, range))|
    \item[D.] \verb|with(d, tapply(age, raceth, diff))|
    \end{description}
  \end{minipage}\hfill
  \begin{minipage}{.35\linewidth}
    Vous répondez :\\ \Square{A}\quad \Square{B}\quad \Square{C}\quad
...
\item[\bf 1.3] On souhaite calculer les déciles pour la variable
  \texttt{age}. Quelle commande faut-il utiliser ? \\[1em]
    \begin{description}
...
\item[\bf 1.5] On souhaite calculer la différence moyenne de pression
  ...
  correctement créée (voir aperçu ci-après, quelle solution peut-on utiliser ?
\begin{verbatim}
> summary(d$aged)
...
linéaire de Bravais-Pearson et un intervalle de confiance à 90~\% ? \\[1em]
\begin{minipage}{.6\linewidth}
  \begin{description}
...
\item[\bf 2.1] On souhaite à présent réaliser une régression linéaire en
    ...
    classe ethnique majoritaire. Quelle commande faut-il utiliser ? \\[1em]
\begin{minipage}{.6\linewidth}
  \begin{description}
  ...
  \begin{minipage}{.35\linewidth}
    Vous répondez :\\ \Square{A}\quad \Square{B}\quad \Square{C}\quad \Square{NSP}
  \end{minipage}
\item[\bf 2.2] En considérant que le modèle décrit en 1.8 a été stocké dans
  une variable appelée \texttt{mod}, que renverrait la commande \verb|anova(mod)| ?     
    \begin{description}
...
\begin{description}
\item[\bf 1.1] Quelle solution peut-on proposer pour stocker ce type de
  données sous R ? \\[1em]
    \begin{description}
...
\item[\bf 1.3] Pour calculer l'intervalle de confiance à 95~\% associé à
  l'odds-ratio estimé en 1.2 et que l'on enregistré dans la variable
  \texttt{tab.or}, on utilise la commande \\[1em] 
  \begin{minipage}{.6\linewidth}
    \begin{description}
...
\item[\bf 1.4] Peut-on retrouver la valeur de l'odds-ratio à partir d'une
  autre commande ? Si oui, laquelle ? \\[1em]
  \begin{minipage}{.6\linewidth}
    \begin{description}
...
\noindent
Table de survie • Estimateur de Kaplan-Meier • Courbe de survie • Test du
 log-rank • Modèle de Cox
\end{minipage}} 
\end{center}
...
\item[(a)] Construire un tableau indiquant le nombre d'exposés et le nombre
  de rechutes en fonction du temps pour chacun des deux groupes.
...
\item[(d)] Calculer la médiane de survie pour chacun des groupes.
