...
Après chaque séance (à l'exception de la première), il est proposé un devoir
composé d'une quinzaine de questions et permettant à l'étudiant de
s'auto-évaluer. Les questions se limitent strictement aux éléments vus en
cours et durant les exercices dirigés.
...
\item[\bf 2.4] Convertir ces deux variables numériques en variables
  ...
  dans chacun des deux cas. \marginpar{2.4 $\square$}
  \begin{description}
  \item[A.] 
\begin{verbatim}
...
\item[\bf 1.4] Le poids moyen des bébés à la naissance, exprimé en grammes, 
  ...
  \item[D.] \verb|by(babies$GENDER, mean(babies$BWEIGHT)|
  \item[E.] Je ne sais pas.
  \end{description}  
\end{description}
...
censurées (sujet dans rechute aux dernières nouvelles) sont signalées par
une astérisque.
\vskip1em
...
de la commande \texttt{survfit}, en limitant les observations aux seuls
membres du groupe 6-MP à l'aide de l'option \texttt{subset}.
<<ex7-2c>>=
...
\verb|fun="cumhaz"| à la commande graphique précédente, soit :
<<ex7-2e, fig=TRUE>>=
plot(s, fun="cumhaz")
@ 
...
fumeur (oui/non), temps et statut (0=vivant, 1=censure). Voici un aperçu de
ces données sous \R :
\begin{verbatim}
> head(sfmhs)
  fumeur temps statut
...
\end{verbatim}
\begin{description}
\item[\bf 1.1] Construire le tableau des événements pour le groupe des non-fumeurs.
\item[\bf 1.2] Afficher la courbe de survie.
\end{description}
\section*{Exercice 2}
