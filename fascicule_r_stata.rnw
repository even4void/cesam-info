...
\centerline{\Huge Initiation aux logiciels R et Stata}
\vskip1em
\centerline{\LARGE Programme de travail}
\vskip2em
\centerline{\includegraphics[scale=.35]{cesam}}
\end{mdframed}
\end{minipage}
\end{center}
\vskip5em
\begin{center}
  \begin{tabular}{ll}
    \textbf{Responsables :} & \\
    Christophe LALANNE & \url{ch.lalanne@gmail.com} \\
...
éléments vus en cours et durant les exercices dirigés.
...
\item[\bf 1.2] Combien de bébés ont une mère ayant eu des antécédents
  d'hypertension ?   \marginpar{1.2 $\square$}
  \begin{description}
...
\item[\bf 1.3] On souhaite afficher la distribution des valeurs prises par
  ...
  la plus appropriée ? \marginpar{1.3 $\square$}
  \begin{description}
...
\begin{description}
\item[\bf 1.6]
Soit l'histogramme de fréquences suivant : \marginpar{1.6 $\square$}
...
\begin{description}
\item[\bf 2.1] Quelles sont les moyennes de $X_1$ et $X_2$ ? \marginpar{2.1 $\square$}
  \begin{description}
  ...
  \end{description}
\item[\bf 2.2] Quelle est l'intervalle inter-quartile de $X_1-X_2$ ?
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 2.3] Donner un intervalle de confiance à 95~\% pour la
  différence $X_1-X_2$ (en utilisant une loi de Student). \marginpar{2.3
    $\square$} 
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.4] On suppose que ces deux variables numériques ont été
  converties en variables qualitatives à 4 classes équilibrées de la manière
  suivante :
\begin{verbatim}
> x1b <- cut(x1, breaks=quantile(x1), include.lowest=TRUE)
> x2b <- cut(x2, breaks=quantile(x2), include.lowest=TRUE) 
\end{verbatim}  
  Indiquer la ou les classes modales dans chacun des deux cas. \marginpar{2.4 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.5] Soit la série de commandes suivantes :
...
@ 
\begin{description}
\item[\bf 3.1] La commande \verb|mean(dfrm$age)| fournit l'âge moyen des
  étudiants. \marginpar{3.1 $\square$}
  \begin{description}
  \item[A.] Vrai.
  \item[B.] Faux.
...
\item[\bf 3.2] On veut remplacer l'âge manquant par l'âge moyen. Laquelle parmi
  ces commandes est correcte ? \marginpar{3.2 $\square$}
  \begin{description}
...
\item[\bf 3.3] On souhaite afficher la proportion de garçons et de filles sous
  ...
  est-elle correcte ? \marginpar{3.3 $\square$}
  \begin{description}
  \item[A.] Vrai.
  \item[B.] Faux.
  \item[C.] Je ne sais pas.
...
\item[\bf 3.4] Pour afficher un histogramme des tailles des garçons, quelle
  commande faut-il utiliser ? \marginpar{3.4 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 3.5] Que fait la commande suivante ? \marginpar{3.5 $\square$}
\begin{verbatim}
...
\begin{description}
\item[\bf 1.1] On souhaite importer les données sous R en utilisant la
  commande \texttt{read.table}. Quelles options faut-il renseigner ?
  ...
  \item[E.] Je ne sais pas
  \end{description}
\item[\bf 1.2] Comment recoder la variable \texttt{GENDER} en variable
  qualitative avec les modalités \texttt{G} (=1) et \texttt{F} (=2) ?
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 1.3] La distribution des effectifs selon le sexe peut être obtenue
  de la manière suivante \marginpar{1.3 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.4] Le poids moyen des bébés à la naissance, exprimé en grammes, 
  chez les sujets des deux sexes peut être obtenu de la manière suivante
...
\begin{description}
\item[\bf 1.5]
Quelle commande permet de reproduire la figure suivante (indépendemment du
rapport largeur/hauteur et de la couleur) ? \marginpar{1.5 $\square$}
\begin{center}
...
\item[\bf 1.6]
Quelle commande permet de reproduire la figure suivante (indépendemment du
rapport largeur/hauteur et de la couleur) ? \marginpar{1.6 $\square$}
\begin{center}
...
\begin{description}
\item[\bf 1.7] 
Voici les résultats d'un test de Student comparant les tailles moyennes des
bébés des deux sexes à la naissance. \marginpar{1.7 $\square$}
\begin{Verbatim}[frame=single]
...
\end{description}
\item[\bf 1.8] 
Le statisticien décide de réaliser un test non-paramétrique à l'aide de la
commande \marginpar{1.8 $\square$}
\begin{verbatim}
...
Veuillez indiquer les commandes permettant de répondre aux questions suivantes :
\begin{description}
\item[\bf 2.1] Pour saisir ces données tabulaires sous R, on peut utiliser
  la commande \marginpar{2.1 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.2] Quelle commande permet d'obtenir la distribution marginale
  chez les femmes (totaux lignes) ? \marginpar{2.2 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 2.3] Le nombre total d'individus peut être obtenu à l'aide de la
  commande \marginpar{2.3 $\square$}
  \begin{description}
...
\begin{description}
\item[\bf 2.4]
La commande \marginpar{2.4 $\square$}
\begin{verbatim}
...
\begin{description}
\item[\bf 2.5]
Voici le résultat d'un test statistique : \marginpar{2.5 $\square$}
\begin{Verbatim}[frame=single]
data:  smoke 
p-value = 1.669e-06
...
\item[\bf 3.1]
Quelle commande permet de reproduire la figure suivante (indépendemment du
rapport largeur/hauteur et de la couleur) ? \marginpar{3.1 $\square$}
\begin{center}
...
\end{description}
\item[\bf 3.2]
La prévalence observée est donnée par : \marginpar{3.2 $\square$}
\begin{description}
...
\end{description}
\item[\bf 3.3]
La commande suivante fournit un intervalle de confiance à 95~\% pour
l'odds-ratio : \marginpar{3.3 $\square$}
\begin{verbatim}
...
\begin{description}
\item[\bf 1.1] Quelle commande a permis de produire le résultat présenté ci-dessus ? \marginpar{1.1 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 1.2] Les données comportent-elles des valeurs manquantes ?  \marginpar{1.2 $\square$}
  \begin{description}
  \item[A.] Oui.
  \item[B.] Non.
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 1.3] On désire recoder la variable \texttt{gain} en variable
  qualitative à quatre classes telles que décrites dans l'énoncé. Quelle
  commande est la plus appropriée ? \marginpar{1.3 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.4] Quelle commande a permis de fournir les résultats suivants : \marginpar{1.4 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.5] Voici le tableau d'analyse de variance où l'on considère
  \texttt{bwt} comme variable réponse et \texttt{gain} (4 classes) comme
  facteur d'étude :  
\begin{verbatim}
...
\end{verbatim}
Quelle est la part de variance expliquée par ce modèle ? \marginpar{1.5 $\square$}
\begin{description}
\item[A.] 8.6~\%.  % SSG/SSE
\item[B.] 21.8~\%. % MSG-MSE
...
\item[D.] Je ne sais pas.
\end{description}  
\item[\bf 1.6] Les données individuelles sont représentées dans le graphique suivant.
\begin{center}
...
l'ANOVA dans ces conditions ? \marginpar{1.6 $\square$}
\begin{description}
...
\end{verbatim}
\begin{description}
\item[\bf 2.1] Pour calculer le rapport entre la variance du groupe 3 et
  celle du groupe 1, on peut utiliser la commande \marginpar{2.1 $\square$}
\begin{description}
...
\item[D.] Je ne sais pas.
\end{description}  
\item[\bf 2.2] On souhaite représenter la distribution de la pression
  systolique à l'aide d'un histogramme pour chacun des trois
  groupes. Quelle commande est la plus appropriée ? \marginpar{2.2 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 2.3] On réalise une ANOVA à l'aide de la commande suivante :
\begin{verbatim}
...
\end{verbatim}
  Est-ce la commande appropriée ?  \marginpar{2.3 $\square$}
  \begin{description}
  \item[A.] Oui.
  \item[B.] Non.
  \item[C.] Je ne sais pas.
  \end{description}  
\item[\bf 2.4] Que réalise la commande suivante ? \marginpar{2.4 $\square$}
\begin{verbatim}
...
\begin{description}
\item[\bf 3.1] En supposant que le tableau de données (10 lignes x 5
  colonnes) aît été stocké dans un \texttt{data.frame} appelé \texttt{peas}
  sous \R, de la manière suivante :
\begin{verbatim}
...
\end{verbatim}
  ...
  d'observations manquantes par traitement ? \marginpar{3.1 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 3.2] Plutôt que de travailler avec un tableau à 5 colonnes, on
  décide de convertir les données en une structure plus facile à manipuler,
  c'est-à-dire un tableau dans lequel on indique dans la première colonne le
  type de traitement auquel les poids ont été soumis, et dans la seconde le
  diamètre des poids. Les données devraient donc ressembler à l'aperçu
  ci-dessous (11 premières observations représentées uniquement) :
\begin{verbatim}
...
\end{verbatim}
  Cette commande est-elle correcte ? \marginpar{3.2 $\square$}
  \begin{description}
  \item[A.] Oui. 
  \item[B.] Non.
  \item[C.] Je ne sais pas.
  \end{description}  
\item[\bf 3.3] On souhaite représenter sous forme d'un diagramme en points
  l'ensemble des mesures individuelles (diamètre, en abscisses) pour chacune
  des conditions expérimentales (traitement, en ordonnées). On supposera que
  le tableau est à présent correctement spécifié sous forme de deux
  colonnes, comme indiqué à l'exercice~3.2. Quelle commande
  est la plus appropriée ? \marginpar{3.3 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 3.4] Voici le résultat d'une ANOVA à un facteur réalisé sur la
  structure de données décrite à l'exercice~3.2 :
\begin{verbatim}
            Df Sum Sq Mean Sq F value   Pr(>F)    
...
\end{verbatim}
  ...
  résiduelle ? \marginpar{3.4 $\square$} 
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 3.5] Quelle commande fournit les moyennes de groupe ?
  \marginpar{3.5 $\square$}  
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 3.6] On souhaite à présent comparer les traitements 2~\%~G et
  2~\%~S. Quelle procédure peut-on utiliser ? \marginpar{3.6 $\square$}
  \begin{description}
  ...
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 3.7] Le résultat obtenu en 3.6 peut être retrouvé à partir de la
  commande : \marginpar{3.7 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 3.8] Supposons que le modèle testé à l'exercice~3.4 ait été stocké
  dans une variable appelée \texttt{m} (\verb|m <- aov(value ~ tx, data=peas)|). 
  Que renverrait la commande suivante ? \marginpar{3.8 $\square$} 
\begin{verbatim}
...
\begin{description}
\item[\bf 1.1] On souhaite afficher un diagramme de dispersion où l'âge
  figure sur l'axe horizontal (abscisses) et la pression systolique sur
  l'axe vertical (ordonnées). Quelle commande peut-on utiliser ? \marginpar{1.1 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 1.2] On souhaite vérifier l'étendue des âges (minimum et maximum)
  par groupe d'ethnicité (variable \texttt{raceth}). Quelle commande permet
  de répondre à cette question ? \marginpar{1.2 $\square$}
  \begin{description}
  \item[A.] \verb|with(d, tapply(age, raceth, c(min, max)))|
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 1.3] On souhaite calculer les déciles pour la variable
  \texttt{age}. Quelle commande faut-il utiliser ? \marginpar{1.3 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.4] On souhaite à présent recoder la variable numérique
  \texttt{age} en une variable qualitative \texttt{aged} définie de sorte
  que chaque valeur numérique observée est remplacée par la valeur du décile
  dans laquelle elle se situe. Supposons que le résultat calculé à
  l'exercice~1.3 ait été stocké dans une variable appelée
...
\end{verbatim}
  ...
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 1.5] On souhaite calculer la différence moyenne de pression
  systolique entre les deux déciles les plus extrêmes (10~\% d'âges les plus
  bas et 10~\% d'âge les plus hauts). En supposant que la variable
  \texttt{aged} (âges individuels recodés sous forme de déciles) ait été
  correctement créée (voir aperçu ci-après), quelle solution peut-on
  utiliser ? \marginpar{1.5 $\square$}
\begin{verbatim}
> summary(d$aged)
...
\end{verbatim}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.6] Quel résultat produit cette commande : \marginpar{1.6 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[C.] Trois graphiques montrant la droite de régression de la
    variable \texttt{age} sur la variable réponse \texttt{SBP} pour pour
    chaque sous-groupe de participants défini selon le facteur ethnicité
    (\texttt{raceth}).
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 1.7]
Voici un diagramme de dispersion résumant la distribution conjointe des
mesures de pression systolique et d'âge. Les déciles d'âge apparaissent sous
forme de disques noirs.
\begin{center}
...
\begin{description}
\item[\bf 2.1] On souhaite à présent réaliser une régression linéaire en
    considérant la variable \texttt{SBP} comme variable réponse et l'âge
    comme variable prédictrice. On souhaite restreindre l'analyse à la
    classe ethnique majoritaire. Quelle commande faut-il utiliser ?
    ...
    \item[D.] Je ne sais pas.
    \end{description}  
\item[\bf 2.2] En considérant que le modèle décrit en 2.1 a été stocké dans
  une variable appelée \texttt{mod}, que renverrait la commande
  \verb|anova(mod)| ? \marginpar{2.2 $\square$}
    \begin{description}
    ...
    \item[D.] Je ne sais pas.
    \end{description}  
\item[\bf 2.3] On souhaite extraire la valeur estimée de la pente de la
  droite de régression à partir des coefficients du modèle. On se propose
  d'utiliser \verb|coef(mod)[j]|. Quelle valeur doit-on spécifier pour
  \texttt{j} ? \marginpar{2.3 $\square$}
  \begin{description}
  \item[A.] 0
  \item[B.] 1
  \item[C.] 2
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.4] En supposant \texttt{j} correctement défini en 2.3, pour
  obtenir un intervalle de confiance à 95~\% pour l'estimé de la pente de
  régression, on utilise la commande : \marginpar{2.4 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 2.5] Quelle hypothèse de la régression linéaire la commande
  graphique suivante permet-elle de vérifier ? \marginpar{2.5 $\square$}  
\begin{verbatim}
...
cm). 
\begin{description}
\item[\bf 3.1] On souhaite afficher les covariations entre ces trois mesures
  sous forme d'une matrice de diagrammes de dispersion. Quelle commande
  doit-on utiliser ? \marginpar{3.1 $\square$}
  \begin{description}
  \item[A.] \verb|xyplot|
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 3.2] Quelle commande permettrait d'afficher les identifiants des
  sujets dont le poids est inférieur (strictement) à 3500 g et la taille
  égale ou supérieure au premier quartile des tailles enregistrées chez ces
  550 bébés ? \marginpar{3.2 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 3.3] En supposant le résultat précédent (liste des identifiants
  des sujets remplissant les conditions mentionnées en 3.2) sauvegardé dans une
  variable appelée \texttt{idx}, comment calculerait-on le périmètre crânien
  moyen de ces individus ? \marginpar{3.3 $\square$}
\begin{verbatim}
> idx
...
\end{verbatim}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 3.4] Voici le résultat des deux régressions des variables
  \texttt{LENGTH} et \texttt{HEADC} sur le poids (\texttt{WEIGHT}) des
  bébés. Quel modèle explique la plus grande part de variance ?
...
\end{verbatim}
  ...
  \item[C.] Je ne sais pas.
  \end{description}  
\item[\bf 3.5] Pour calculer le poids attendu d'un bébé ayant un périmètre
  crânien de 37 cm (1\ier quartile des périmètres crâniens observés dans
  l'échantillon), quelle commande doit-on utiliser ? \marginpar{3.5 $\square$}
  \begin{description}
...
L'objet de l'analyse de vérifier si l'obscurcissement de la cornée constitue
effectivement un facteur prédictif du risque de développer un infarctus.
\begin{description}
\item[\bf 1.1] En notant \texttt{M+/M-} les cas et les témoins, et
  \texttt{E+/E-} les personnes exposées/non-exposées, quelle solution
  peut-on proposer pour stocker ce type de données sous \R ? \marginpar{1.1 $\square$} 
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 1.2] Supposons que le tableau de données ait été appelé
  \texttt{tab} sous \R. On souhaite calculer l'odds-ratio à l'aide de la
  commande \texttt{oddsratio} du package \texttt{vcd}. Quelle est la
  commande à utiliser ? \marginpar{1.2 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 1.3] Pour calculer l'intervalle de confiance à 95~\% associé à
  l'odds-ratio estimé en 1.2 et enregistré dans la variable
  \texttt{tab.or}, on utilise la commande : \marginpar{1.3 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}  
\item[\bf 1.4] Peut-on retrouver la valeur de l'odds-ratio à partir d'une
  autre commande ? Si oui, laquelle ? \marginpar{1.4 $\square$}
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.5] Quelle commande utiliserait-on pour représenter graphiquement
  la fréquence relative des cas (uniquement) selon le facteur d'exposition ?
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 1.6] Que renvoit la commande suivante ? \marginpar{1.6 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 1.7] Quelle commande du package \texttt{epiR} permet de fournir
  une estimation par intervalle (95~\%) de la prévalence observée sur cet
  échantillon ?  \marginpar{1.7 $\square$}
  \begin{description}
...
\begin{description}
\item[\bf 2.1] On se propose dans un premier temps de transformer le tableau
  de contingence sous forme d'un \texttt{data.frame} comportant trois
  colonnes décrivant le statut relatif à la maladie (M+/M-) et à
  l'exposition (E+/E-) et les effectifs associés. La commande
  suivante est-elle correcte ? \marginpar{2.1 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 2.2] En supposant que les données sont à présent correctement
  spécifiées telles que décrit en 2.1, soit
\begin{verbatim}
> tab
...
\end{verbatim}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.3] Pour afficher le résultat des tests de nullité des
  coefficients du modèle (ordonnée à l'origine et pente), on utilise la
  commande : \marginpar{2.3 $\square$}
  \begin{description}
  \item[A.] \verb|summary|
  \item[B.] \verb|anova|
  \item[C.] Je ne sais pas.
  \end{description}  
\item[\bf 2.4] On se propose de retrouver la valeur de l'odds-ratio calculé
  à l'exercice~1.2  à partir du modèle précédent. Quelle combinaison de
  commandes est nécessaire pour réaliser cette opération ? \marginpar{2.4 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.5] On souhaite calculer la probabilité d'être un cas sachant que
  l'on a été exposé. Quelle commande permet de répondre à cette question ?
  \marginpar{2.5 $\square$}
  \begin{description}
...
chirurgicale intervenant après l'évaluation a permis de confirmer la
...
tab[2,2]/sum(tab[2,])
\end{verbatim}
  fournit une estimation de la valeur prédictive positive ? \marginpar{3.1 $\square$} 
  \begin{description}
  \item[A.] Vrai.
  \item[B.] Faux.
  \item[C.] Je ne sais pas.
  \end{description}  
\item[\bf 3.2] Soit la valeur de VPN=0.91 et la commande suivante :
\begin{verbatim}
0.91 + 1.96 * sqrt(0.91*0.09/150)
\end{verbatim}
  Que produit cette commande ? \marginpar{3.2 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 3.3] Peut-on obtenir un résultat comparable à partir d'une autre
  commande \R ? \marginpar{3.3 $\square$}
  \begin{description}
  \item[A.] \verb|fisher.test|
  \item[B.] \verb|prop.test|
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 3.4] Voici le contenu partiel d'une sortie produite par \R :
\begin{verbatim}
...
Sida jusqu'au décès du participants ou à la fin de la durée de suivi
...
\end{verbatim}
\begin{description}
\item[\bf 1.1] La commande suivante permet-elle d'indiquer la proportion de
  sujets fumeurs encore en vie à la fin de l'étude ? \marginpar{1.1 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 1.2] On souhaite afficher la table de mortalité des
  non-fumeurs. Quelle commande doit-on utiliser ? \marginpar{1.2 $\square$} 
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 1.3] En notant \texttt{st} la variable dans laquelle on a stocké
  les données de survie enregistrées au format \R via la commande
  \texttt{Surv}, la commande suivante permet de fournir le même résultat que
  la commande de l'exercice~1.2 ?  \marginpar{1.3 $\square$}
\begin{verbatim}
...
\end{verbatim}  
  ...
  \item[C.] Je ne sais pas.
  \end{description}
\item[\bf 1.4] Pour afficher la courbe de survie du groupe des patients
  non-fumeurs, sans intervalles de confiance, quelle commande peut-on
  utiliser ? On supposera que la table de mortalité a été stockée dans une
  variable appelée \texttt{stab}. \marginpar{1.4 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 1.5] À quelle procédure de test correspond la commande suivante ?
  \marginpar{1.5 $\square$}
\begin{verbatim}
...
la cellularité est inférieure à 100~\%. Les censures (patients décédés à la
fin de la période de suivie) sont indiquées par le symbole \texttt{+}, et
les durées sont reportées en mois.
\vskip1em
...
\begin{description}
\item[\bf 2.1] Quelle solution peut-on proposer pour importer ces données
  sous \R ? \marginpar{2.1 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.2] On supposera par la suite que les données ont été importées
  et stockées dans une variable appelée \texttt{aml}. Pour travailler avec
  ces données sous \R, il est nécessaire de recoder la variable
  \texttt{status} de façon à ce que les valeurs 0 soient transformées en 1
  et, réciproquement, les valeurs 1 en 0. Quelle commande ou combinaison de
  commandes permet de réaliser cette opération ? \marginpar{2.2 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.3]  Que produit la commande suivante ? \marginpar{2.3 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.4] Le nombre de données censurées par groupe d'âge peut être
  représenté graphiquement sous forme d'un diagramme en points à l'aide de
  la commande : \marginpar{2.4 $\square$} 
  \begin{description}
  ...
  \item[E.] Je ne sais pas.
  \end{description}  
\item[\bf 2.5] On souhaite effectuer une régression de Cox pour étudier
  l'effet du facteur âge. Quelle commande est la plus appropriée ?
  \marginpar{2.5 $\square$}
  \begin{description}
  ...
  \item[D.] Je ne sais pas.
  \end{description}
\item[\bf 2.6] Voici les résultats retournés par \R pour un modèle de Cox
  incluant les deux prédicteurs :  \marginpar{2.6 $\square$}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item[E.] Je ne sais pas.
  \end{description}
\item[\bf 2.7] La commande \texttt{confint}, appliquée directement sur le
  modèle \texttt{coxph(...)} renverra : \marginpar{2.7 $\square$}
  \begin{description}
...
\centerline{\small Centre d'Enseignement de la Statistique Appliquée, à la
  Médecine et à la Biologie Médicale} 
\vspace*{5cm}
\begin{center}
\begin{minipage}{.75\textwidth}
\begin{mdframed}[style=titlep]
\centerline{\Huge Initiation aux logiciels R et Stata}
\vskip1em
\centerline{\LARGE Corrigés des exercices}
\vskip2em
\centerline{\includegraphics[scale=.35]{cesam}}
\end{mdframed}
\end{minipage}
